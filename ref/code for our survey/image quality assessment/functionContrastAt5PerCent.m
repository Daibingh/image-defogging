...
function [Mask Crr Mask_1 Crr_1] = functionContrastAt5PerCent(I1,S,percentage)
% Inputs 
% I1 : Original image
...
Mask_1 = false(nl+2*S,nc+2*S); 
Crr_1  = zeros(nl+2*S,nc+2*S);  
...
for ii = 1:round(S/2):nl
    for jj = 1:round(S/2):nc
        ...
        if(M>0)
            ...
            Mask_1(S+ii:2*S+ii-1,S+jj:2*S+jj-1) =  Mask_1(S+ii:2*S+ii-1,S+jj:2*S+jj-1) |  Fcube(:,:,s0-Ismin+1);
            I3_1 = Mask_1(S+ii:2*S+ii-1,S+jj:2*S+jj-1);
            % recover the contrast value of visible edge
            Crr1_1       = zeros(S,S);
            Crr1_1(I3_1) = 2*M/256;
            Crr_1(S+ii:2*S+ii-1,S+jj:2*S+jj-1) = Crr1_1;
        end
